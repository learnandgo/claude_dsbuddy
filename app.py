...
.chat-container {
    ...
    padding: 1.5rem; margin-bottom: 1rem; min-height: 420px; max-height: 580px; overflow-y: auto;
}
...
/* ── Question header card (for structured session) ── */
.q-header {
...
.q-text { color: white; font-size: 0.85rem; font-weight: 500; }
...
.fb-score-sub { font-size: 0.75rem; color: #6366f1; margin-top: 2px; }
...
.score-bar-bg { background: #1e293b; border-radius: 99px; height: 8px; overflow: hidden; margin-top: 4px; }
.score-bar-fill { height: 8px; border-radius: 99px; transition: width 0.5s; }
</style>
...
<div class="ds-header">
    ...
    <p>Practice Data Science, Analytics, Engineering & AI interviews with structured AI feedback and scoring</p>
</div>
""", unsafe_allow_html=True)

# ── Controls — Topic + Difficulty + Apply (no Mode selector) ─────────────────
...
if apply:
    ...
    st.session_state.session_questions_used = []
    st.session_state.input_key += 1
    st.rerun()

st.markdown("<div style='margin:0.75rem 0'></div>", unsafe_allow_html=True)

# ── Stats bar — 3 cards: Interactions, Avg Score, Session Progress ────────────
...
if st.session_state.session_active:
    session_display = f"{st.session_state.session_qs_done}/{st.session_state.session_total}"
    session_label = "Session Progress"
else:
    session_display = "—"
    session_label = "Session Progress"
...
with s3:
    ...
    st.markdown(f"""<div class="stat-card session">
        ...
        <div class="stat-label">{session_label} ({qs_left} left)</div>
    </div>""", unsafe_allow_html=True)
...
def build_system_prompt(mode="free"):
    ...
    feedback_format = """When evaluating an answer, ALWAYS use EXACTLY this structure — no exceptions:
...
Your model answer here."""
    ...
    if mode == "session":
        mode_instr = f"""You are running a structured 5-question interview session.
You will be given a specific question to ask. Ask it clearly, labelled as the question number provided.
After the user answers, give structured feedback using the format below.
Then confirm you are moving to the next question.
{feedback_format}"""
    else:
        mode_instr = f"""You are in free chat mode.
Answer any questions about {topic}. When asked for an interview question, ask one and wait.
When the user answers, give structured feedback using the format below.
{feedback_format}"""
...
# ── Pick shuffled questions for session ───────────────────────────────────────
def pick_session_questions():
    pool = QUESTION_BANK[st.session_state.applied_topic][st.session_state.applied_diff]
    shuffled = random.sample(pool, min(5, len(pool)))
    st.session_state.session_questions_used = shuffled
    return shuffled
            ...
            if stripped.upper().startswith(key):
                ...
                if block_type == "score":
                    ...
                    try:
                        ...
                        bar_width = score_num * 10
                        bar_color = "#22c55e" if score_num >= 7 else "#f59e0b" if score_num >= 5 else "#ef4444"
                    except:
                        bar_width = 0
                        bar_color = "#94a3b8"
                    ...
                    html_out += f"""<div class="fb-block score">
                        ...
                        <div class="score-bar-wrap">
                            <div class="score-bar-bg"><div class="score-bar-fill" style="width:{bar_width}%;background:{bar_color}"></div></div>
                        </div>
                    <div>"""
                else:
                    ...
                    if inline:
                        if block_type == "model":
                            ...
                            sym = "●" if block_type == "good" else "●"
                            html_out += f'<div class="fb-point"><span class="{dot}">{sym}</span><span class="fb-point-text">{inline}</span></div>'
...
# ── Render session question header ────────────────────────────────────────────
def session_q_header(q_num, total, question_text):
    return f"""<div class="q-header">
        <span class="q-badge">Question {q_num} of {total}</span>
...
if not st.session_state.messages:
    chat_html += """<div class="msg-assistant"><div>
        ...
        <div class="bubble-assistant">
            ...
            <b>Free Chat</b> — ask anything, request a question, get explanations<br>
            <b>5-Question Session</b> — structured interview with scoring and final summary
        </div>
    </div></div>"""
else:
...
if send and user_input.strip():
    ...
    questions = pick_session_questions()
    ...
    send_message(
        f"Start a structured 5-question {st.session_state.applied_diff} {st.session_state.applied_topic} interview session.\n"
        f"Use these exact questions in this order:\n{q_list}\n"
        f"Ask Question 1 of 5 now. Format it clearly as 'Question 1 of 5:' followed by the question.",
        "session"
    )
